...
func UpdateCategories(c *gin.Context) {
	...
	go events.Publish("categories.updated", map[string]string{"count": fmt.Sprintf("%d", len(cats)), "timestamp": time.Now().Format(time.RFC3339)})
}
...
func UpdateSettings(c *gin.Context) {
	...
	go events.Publish("settings.updated", map[string]string{"timestamp": time.Now().Format(time.RFC3339)})
}
