...
2. 日志上下文中的 HH:MM 默认表示事项的结束时间，不是开始时间；不要把结束时间点误判为开始时间点。
3. 推断时长时，结束时间点模式下某条事项的开始时间通常来自上一条日志时间；没有上一条记录时起点未知。
4. 开头先给出一句整体概览。
5. 再用 2 到 4 个简短小节或要点，指出时间投入重点、节奏特征和可观察到的问题。
6. 语气客观、简洁、可读，适合消息通知、日报、周报或月报推送。
7. 如果记录较少，要明确指出样本有限。`,
		Builtin: true,
	},
	{
...
日志上下文中的 HH:MM 默认表示事项的结束时间，不是开始时间；结束时间点模式下，某条事项的开始时间通常由上一条日志时间推断。请不要把结束时间点误判为开始时间点。
...
2. 日志上下文中的 HH:MM 默认表示事项的结束时间，不是开始时间；识别规律时请按完成/结束时间理解这些时间点。
3. 每个检查点包含：name（英文标识）、description（中文描述）、expected_time（建议检查时间，HH:MM 格式，应晚于用户通常完成该活动的时间 15~30 分钟）、time_window_minutes（检查时间窗口，分钟）、event_patterns（匹配的事项关键词数组）、category（所属分类）、message（提醒消息模板）。
4. 只输出 JSON 数组，不要包含其他文字。
5. 至少需要在 3 天以上观察到的规律才生成检查点。
6. message 中应包含用户通常执行该活动的时间范围。`,
		Builtin: true,
	},
}
